...
mainwindow.iconbitmap('{}\\icon\\paintdotnet.ico'.format(dir_path)) #아이콘 설정
...
def func_BrightnessAndcontrastAdjustment():
    ...
    bright_Scale=tkinter.Scale(BC_control,label="Brightness -100~100",variable=bright_var, orient="horizontal", showvalue=True, tickinterval=20, from_=-100, to=100, length=300)
    contrast_Scale = tkinter.Scale(BC_control,label="Contrast(1/10) 1.0~3.0",variable=contrast_var, orient="horizontal", showvalue=True, tickinterval=2, from_=10, to=30, length=300)
    bac_Button = tkinter.Button(BC_control,text="Apply",command=func_BandC)
...
def func_HueAndSaturationAdjustment():
    ...
    HSV_control.title("H,S,V adjustment")
...
def func_PosterizeEffect():
    ...
    Poster_control.title("PosterizeEffect")
...
def func_DesaturateEffect():
    global Photo_list, photo_number
    B_image = ImageOps.grayscale(Photo_list[photo_number])
    newX = B_image.width
    newY = B_image.height
    displayImage(B_image, newX, newY)
...
for tn in MenuFile_lst:
    globals()["icon_{}".format(tn)] = tkinter.PhotoImage(
        file="{}\\icon\\File\\MenuFile{}Icon.192.png".format(dir_path,tn))
...
for tn in MenuEdit_lst:
    globals()["icon_{}".format(tn)] = tkinter.PhotoImage(
        file="{}\\icon\\Edit\\MenuEdit{}Icon.192.png".format(dir_path,tn))
...
for tn in MenuView_lst:
    globals()["icon_{}".format(tn)] = tkinter.PhotoImage(
        file="{}\\icon\\View\\MenuView{}Icon.192.png".format(dir_path,tn))
...
for tn in MenuImage_lst:
    globals()["icon_{}".format(tn)] = tkinter.PhotoImage(
        file="{}\\icon\\Image\\MenuImage{}Icon.192.png".format(dir_path,tn))
...
for tn in MenuAdjustments_lst:
    globals()["icon_{}".format(tn)] = tkinter.PhotoImage(
        file="{}\\icon\\Adjustments\\{}.192.png".format(dir_path,tn))
...
# 도구 보조창  아이콘출력
icon_MenuWindowTools = tkinter.PhotoImage(file="{}\\icon\\utility\\MenuWindowToolsIcon.192.png".format(dir_path)) #툴 아이콘 불러오기
...
def func_Text():
    ...
    def text_process(event):
        ...
        newWindow.title("TextInput")
        newWindow.geometry("300x100")
        newWindow.geometry("+%d+%d" %(mainwindow.winfo_x()+82,mainwindow.winfo_y()+140))
        text_entry = tkinter.Entry(newWindow)

        button = tkinter.Button(newWindow,text="입력",command=input_text)

        fonts=["굴림","궁서","맑은 고딕"]
        font_combobox = tkinter.ttk.Combobox(newWindow,textvariable="폰트",values=fonts,width=10)

        sizes=[]
        for i in range(100):
            sizes.append(i)
            
        font_size_combobox= tkinter.ttk.Combobox(newWindow,values=sizes,width=10)
...
for tn in Tool_lst:
    globals()["icon_{}".format(tn)] = tkinter.PhotoImage(
        file="{}\\icon\\Tool\\{}ToolIcon.192.png".format(dir_path,tn))
